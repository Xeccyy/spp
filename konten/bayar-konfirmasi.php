                ...
                <div class="card-body">
                    <form action="aksi/bayar.php" method="post" enctype="multipart/form-data">
                        ...
                        <select name="id_bayar_metode" class="form-control" readonly>
                            ...
                            $sql_metode = "SELECT * FROM bayar_metode WHERE dihapus_pada IS NULL AND 
                            id_bayar_metode=$id_bayar_metode";
                        ...
                        <label for="keterangan">Keterangan Pembyaran</label>
                        <textarea name="keterangan" class="form-control" rows="3" placeholder="Isi Dengan keterangan pembayaran spserti nama bank,nama,pengirim,dll" readonly><?= $bayar['keterangan']; ?></textarea>
                        ...
                        <button type="submit" class="btn btn-info btn-block mt-2"onclick="return confirm('Apakah Anda Yakin Semua Data Benar?')"><i class="fas fa-save"></i>Konfirmasi Pembayaran</button>
                    </form>
                </div>
            </div>
            <!-- /.container-fluid -->
