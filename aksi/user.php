...
if ($_POST) {
    if ($_POST['aksi'] == 'tambah') {
        ...
        $sql = "INSERT INTO user (id_user,nama,username,password,hak_akses,dibuat_pada,diubah_pada,dihapus_pada) VALUES (DEFAULT,'$nama','$username','$password','$akses',DEFAULT,DEFAULT,DEFAULT)";
...
if ($_GET) {
    if ($_GET['aksi'] == 'hapus') {
        ...
        // HARD DELETE 
        $sql = "UPDATE user SET dihapus_pada=now() WHERE
        id_user=$id_user"; // Soft Delete 
        ...
        $id_user = $_GET['id_user'];
        $sql = "UPDATE user SET dihapus_pada= NULL WHERE
        id_user=$id_user";
        mysqli_query($koneksi, $sql);
        notifikasi($koneksi);
        header('location:../index.php?p=user');
