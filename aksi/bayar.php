...
if ($_POST) {
    if ($_POST['aksi'] == 'tambah') { // Untuk manajemen
        ...
        $nama_file_bukti = $date . "." . $_FILES['bukti']['name'];
        ...
        $query_cari_id_bayar = mysqli_query(
            $koneksi,
            $sql_cari_id_bayar
        );
        ...
        while ($tagihan = mysqli_fetch_array($query_tagihan)) {
            ...
            // Update Tagihan
            $sql_update_tagihan = "UPDATE tagihan SET 
                total_terbayar=total_terbayar+$dibayarkan 
    ...
    } else if ($_POST['aksi'] == 'konfirmasi-bayar') { // Konfrmasi bayar
        ...
        $sql = "UPDATE biaya SET id_periode='$id_periode',
            deskripsi_biaya='$deskripsi_biaya',tingkat='$tingkat',id_jurusan='$id_jurusan',jumlah_biaya='$jumlah_biaya',
...
if ($_GET) {
    if ($_GET['aksi'] == 'hapus') {
        ...
        // HARD DELETE 
        $sql = "UPDATE bayar SET dihapus_pada=now() WHERE
        id_bayar=$id_bayar"; // Soft Delete 

        mysqli_query($koneksi, $sql);

        $sql2 = "UPDATE bayar_alokasi SET dihapus_pada=now()WHERE
        id_bayar=$id_bayar";
        mysqli_query($koneksi, $sql2);

        // Update Saldo Tagihan
        $sql_tagihan = "SELECT * FROM bayar_alokasi WHERE 
        id_bayar=$id_bayar";
        $query3 = mysqli_query($koneksi, $sql3);
        while ($row3 = mysqli_fetch_array($query3)) {
            $id_tagihan = $row['id_tagihan'];
            $total_alokasi = $row3['total_alokasi'];
            $sql4 = "UPDATE tagihan SET total_terbayar=total_terbayar-$total_alokasi
            WHERE id_tagihan=$id_tagihan";
        ...
        $id_bayar = $_GET['id_bayar'];
        $sql = "UPDATE bayar SET dihapus_pada= NULL WHERE
        id_bayar=$id_bayar";
        mysqli_query($koneksi, $sql);
        notifikasi($koneksi);
        header('location:../index.php?p=bayar');
